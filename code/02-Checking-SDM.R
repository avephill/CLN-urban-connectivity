...
tt_pred.strs <- read_stars(paste0(results.dir, "Taricha_torosa/prediction.tif"))
...
tt.g <-
  ggplot() +
  geom_stars(data = tt_pred.strs, aes(fill = prediction.tif),
             na.action = na.omit) +
...
varimp.plot <- ggplot(tt_varimp,
                        aes(forcats::fct_reorder(Variable, Permutation_importance, .desc = T),
...
library(glmnet)
library(SDMtune)
sdm_mod <- readRDS(paste0(results.dir, "Taricha_torosa/sdm_model.rds"))
auc(sdm_mod)
tss(sdm_mod)
