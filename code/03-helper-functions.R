...
# Omniscape ini file ---------------------------------------
create_omniscape_ini <- function(output_path, resistance_file, source_file) {
  norm_output_path <- output_path |> normalizePath() |> paste0("/")
  write_path <- norm_output_path |>
    paste0(format(Sys.time(), "%Y-%m-%d_%H:%M"),"_omniscape")
  # browser()
...
write_raw_currmap = true
  ...
  ini_path <- paste0(norm_output_path, "_config.ini")
